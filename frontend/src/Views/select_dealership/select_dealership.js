...
componentDidUpdate(){
 ...
 renderCarList(){
    ...
    return dealerships.map((item)=>( 
      ...
      <div className=" card hover:shadow-lg flex flex-row rounded-md   bg-white overflow-hidden shadow-md my-2 w-3/4 h-60 mx-auto " key={item.id}>
        ...
        <div className=" flex font-sans ml-4 mt-3 w-full">
       ...
       <Link to={`/catalog/${item.id}`}> <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-2 mx-2 rounded">
        Select
       </button> </Link>
     </div>  
 ...
 render(){ 
    ...
    const {car}=this.props.dealerships
     return( 
         ...
         <div className=" card hover:shadow-lg w-1/2 h-1/3 mt-14 ml-12 shadow-md bg-white overflow-hidden ">
         ...
         <span className="text-sm text-grey-500 mt-6">{car.price} EGP</span>
         </div>
         </div>
         ...
         <div className="justify-start mx-auto"> <h1 className="text-3xl font-bold "> Select Dealer</h1></div>   
         {this.renderCarList()}
